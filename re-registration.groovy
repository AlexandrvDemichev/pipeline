            ...
            steps{
                script{
                    if(currentBuild.result!='ABORTED'){
                        ...
                        try{
                         def resp = sh(script: 'curl -v -X POST "127.0.0.1:5000/api/re-registration?id=${id_system}&version=${version}&path=Nexus_PROD/${id_system}/${version}/${id_system}-${version}-distrib.zip" ', returnStdout: true)        
